...
Watcher.prototype.init = function(stream){
    ...
    function onData(response){
        ...
        debug('watcher-%s got events,length:%d',self.id,events.length);
        //events
...
function _inited(w) {
    var request = null;
    while (w.waitQueue.length>0) {
        _write(w,w.waitQueue.shift());
    }
}
...
function _write(w,request){
    //not ready
    if(!w.stream){
        w.waitQueue.push(request);
        debug('watcher-%s cached request,current length:%d',w.id,w.waitQueue.length);
    }else{
