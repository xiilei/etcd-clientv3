...
Watcher.prototype.init = function(){
    ...
    function onData(response){
        ...
        if(response.getCreated()){
            var event = this.retry ? 're-create' : 'create';
            debug('watcher-%s %s',this.id,event);
            this.emit(event,this.id);
            return;
        }
        if(response.getCanceled()){
            var event = this.retry ? 're-cancel' : 'cancel';
            debug('watcher-%s %s',this.id,event);
            this.emit(event,this.id);
            return;
        }
        ...
        if(debug.enabled){
            debug('watcher-%s got events,length:%d,compact_revision:%s',
                this.id,events.length,response.getCompactRevision());
        }
        //events
        this.emit('events', _.reduce(events,function(arr,event){
            var kv = event.getKv();
    ...
    this.stream.once('end',onEnd.bind(this));
    this.stream.on('error',onError.bind(this));
    this.cleanFns.push(function(){
        debug('watcher-%s cleanup',self.id);
        self.stream.removeListener('data',onData.bind(this));
        self.stream.removeListener('end',onEnd.bind(this));
        //uncatch error
        self.stream.removeListener('error',onError.bind(this));
    });
