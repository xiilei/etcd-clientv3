...
function Service(conn){
    ...
    this.reconnectWhenStatus = [grpc.status.UNAVAILABLE];
 ...
 * do request
 */
function doRequest(agent,method,request,callback){
    callback = callback || _.noop;
    if(agent.ready){
        debug('%s do %s',agent.name,method);
        agent.conn[method](request,function(err,response){
            if(err && ~agent.reconnectWhenStatus.indexOf(err.code)){
                ...
                agent.ready = false;
                _putOfflineQueue(agent,method,request,callback);
 ...
 */
function _putOfflineQueue(agent,method,request,callback){
    ...
    if(agent.highWaterMark>agent.offlineQueue.length){
        agent.offlineQueue.push(doRequest.bind(this,agent,method,request,callback));
